...
if st.session_state.research_results:
    ...
    with col_left:
        ...
        with dl_col2:
            html_report = f"<!DOCTYPE html><html><head><title>Research Report</title><style>body{{font-family:sans-serif;padding:30px;line-height:1.6;background:#050d1a;color:#e2e8f0;}} pre{{background:#1e293b;padding:15px;border-radius:8px;}} a{{color:#38bdf8;}}</style></head><body><pre>{md_report}</pre></body></html>"
            st.download_button(
                label="🌐 Download HTML Report (.html)",
                data=html_report,
                file_name=f"research_report_{res.get('request_id', 'output')}.html",
                mime="text/html",
                use_container_width=True
            )
